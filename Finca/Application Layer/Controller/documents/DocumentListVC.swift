...
class DocumentListVC: BaseVC,AppDialogDelegate {
    @IBOutlet weak var viewOfSearch: UIView!
    @IBOutlet weak var ivbtnadd: UIView!
    ...
    func btnAgreeClicked(dialogType: DialogStyle,tag : Int) {
        if dialogType == .Delete{
            self.dismiss(animated: true) { [self] in
                ...
                self.dodeleteMydocument(document_id: self.filterDocumentList[tag].documentID!)
...
////
        if filterDocumentList[indexPath.row].documentTypeID == "0"{
            ivbtnadd.isHidden = false
        }
        ...
        if filterDocumentList[indexPath.row].userId == doGetLocalDataUser().userID
        {
            ...
            cell.heightbtndelete.constant = 30
...
//               UIApplication.shared.open(url)
    ...
    @objc func btnDelete(sender : UIButton) {
     ...
     func dodeleteMydocument(document_id : String) {
