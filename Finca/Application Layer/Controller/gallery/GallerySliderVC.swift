...
class GallerySliderVC: BaseVC {
    ...
    override func viewWillAppear(_ animated: Bool) {
        pager.scrollToItem(at: index, animated: false)
        pager.reloadData()
    }
    
    @IBAction func btnBackPressed(_ sender: UIButton) {
        event_Image_Array.removeAll()
        pager.reloadData()
       // cvImageSlider.reloadData()
        self.navigationController?.popViewController(animated: true)
    }
    @IBAction func btnShareClicked(_ sender: UIButton) {
        let imagestring = event_Image_Array[pager.currentItemIndex].galleryPhoto!
        let url = imagestring.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
        var downloadedImage : UIImage!

        AF.request(url).responseData(completionHandler: { response in
            if let image = UIImage(data: response.data!){
                downloadedImage = image
                let shareAll = [ downloadedImage as Any ] as [Any]
                ...
                self.toast(message: "Image Sharing Failed. Try Again!!", type: .Faliure)
            }
        })
    ...
    @IBAction func btnDownloadClicked(_ sender: UIButton) {
        let imagestring =  String(event_Image_Array[pager.currentItemIndex].galleryPhoto!)
...
extension GallerySliderVC : UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        Utils.setImageFromUrl(imageView: cell.imgEvent, urlString: event_Image_Array[indexPath.row].galleryPhoto, palceHolder: "fincasys_notext")
...
extension GallerySliderVC : iCarouselDelegate,iCarouselDataSource {
    ...
    func carousel(_ carousel: iCarousel, viewForItemAt index: Int, reusing view: UIView?) -> UIView {
        ...
        Utils.setImageFromUrl(imageView: viewCard.imgGallerySlider, urlString: event_Image_Array[index].galleryPhoto, palceHolder: "banner_placeholder")
