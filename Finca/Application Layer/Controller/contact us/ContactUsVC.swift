...
class ContactUsVC: BaseVC {
    ...
    override func viewDidLoad() {
        ...
        ivEmail.setImageColor(color: #colorLiteral(red: 0.1529411765, green: 0.5568627451, blue: 0.8862745098, alpha: 1))
          ivCall.setImageColor(color: #colorLiteral(red: 0.1529411765, green: 0.5568627451, blue: 0.8862745098, alpha: 1))
    ...
    @IBAction func onClickSubmit(_ sender: Any) {
        ...
        if tfFeedback.text!.count < 9 {
            toast(message: "Enter feedback minimum 10 charecter", type: .Faliure)
        } else {
            
            if doGetLocalDataUser().userEmail != nil {
                  sendFeedback()
            }else {
                showAlertMessage(title: "", msg: "Update Email in Your Profile To Add Feedback")
            }
